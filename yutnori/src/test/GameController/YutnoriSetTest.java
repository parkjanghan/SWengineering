...
class YutnoriSetTest {
    ...
    @Test
    void rollYutTest() { //자동으로 윷을 던짐
        yutnoriSet.setPlayerTurn(1);//1번 사용자의 턴
        YutResult rollResult = yutnoriSet.getPlayerResults().getFirst();
        assertNotNull(rollResult);
        assertTrue(rollResult == YutResult.GAE || rollResult == YutResult.YUT ||
                rollResult == YutResult.DO || rollResult == YutResult.GEOL ||
                rollResult == YutResult.BACK_DO|| rollResult == YutResult.MO);
        System.out.println("Roll Result: " + rollResult);
        System.out.println("InGameFlag: " + yutnoriSet.getInGameFlag());

    }

    @Test
    void rollYutforTest_test()
    {
        yutnoriSet.setPlayerTurn(1);//1번 사용자의 턴
        YutResult rollResult = yutnoriSet.getPlayerResults().getFirst();
        assertNotNull(rollResult);
    ...
    void tryStackMal() {
        ...
        assertEquals(2, yutnoriSet.players.get(0).getMalList().get(0).getPosition()); //0번 말이 2번 노드로
        assertEquals(2, yutnoriSet.players.get(0).getMalList().get(1).getPosition()); //1번 말이 2번 노드로
        assertEquals(2, yutnoriSet.board.boardShape.get(2).getNumOfOccupyingPieces());
//        System.out.println("Board: " + yutnoriSet.board);
//        System.out.println("BoardShape: " + yutnoriSet.board.boardShape);
//        System.out.println("Players: " + yutnoriSet.players);
//        System.out.println("Node(2): " + yutnoriSet.board.boardShape.get(2));
    ...
    void selectBoardPieceTest()
    {
       ...
       int unstartedMalID= yutnoriSet.selectOutOfBoardPiece(0);
        ...
        assertEquals(3, yutnoriSet.players.get(0).
        ...
        assertEquals(1, yutnoriSet.players.get(1).
                getMalList().get(0).getPosition()); //1번 플레이어는 아무런 말도 아직 안
        //움직인 상태이다!
        ...
        boolean player0Captured = yutnoriSet.tryCatchMal(1, 3);
